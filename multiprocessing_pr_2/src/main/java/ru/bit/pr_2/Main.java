...
public class Main {
    public static void main(String[] args) {
        ...
        SeqBFS warmupSeqBFS = new SeqBFS(graph, source);
        warmupSeqBFS.execute();

        ParBFS warmupParBFS = new ParBFS(graph, source, numThreads);
        warmupParBFS.execute();
        System.out.println("Разогревочные запуски завершены.\n");

        SeqBFS seqBFS = new SeqBFS(graph, source);
        long seqTotalTime = 0;
        ...
        ParBFS parBFS = new ParBFS(graph, source, numThreads);
        long parTotalTime = 0;
